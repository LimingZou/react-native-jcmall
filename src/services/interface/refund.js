...
const refundList = require("./data/refund");
const refundInfo = require("./data/refund_detail");
...
export class RefundListInfoInterface extends Interface {
  ...
  constructor(param) {
    ...
    try {
      ...
      this.goods_spec_string = goods_spec_string
        ? goods_spec_string.join(",")
        : null;

      if (param.handle_expiry_time) {
        this.handle_expiry_seconds = parseInt(
          param.handle_expiry_time - Date.now() / 1000
        );
      }
      if (param.send_expiry_time) {
        this.send_expiry_seconds = parseInt(
          param.send_expiry_time - Date.now() / 1000
        );
      }
    } catch (e) {
      throw new Exception(
        e,
        "RefundListInfoInterface interface attribute error"
      );
    }
  }
}
