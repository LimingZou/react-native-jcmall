      ...
      <View style={{ flex: 1 }}>
        <View style={styles.popModalTitleView}>
          ...
          <View style={styles.popModalTitleTight}>
            <Text style={[styles.popModalTitleTightP]}>
              {" "}
              ¥{current_sku ? current_sku.price : 0}
            </Text>
            <Text style={[PublicStyles.descTwo9]}>
              已选：
              {current_sku && current_sku.spec && current_sku.spec[0].id
                ? current_sku.spec.map(item => {
        ...
        <ScrollView style={styles.SpecListView}>
          {spec_list && spec_list.length && spec_list[0].id
            ? spec_list.map((spec_list_item, i) => {
                const family_selected_data = spec_list_item.value_list.find(
                  brotherItem => {
                ...
                return (
                  ...
                  >
                    <View
                      style={[PublicStyles.rowCenter, { marginBottom: 18 }]}
                    >
                      ...
                      {family_selected_data ? null : (
                        <Text
                          style={[
                            PublicStyles.descTwo6,
                            { marginLeft: 10, color: ThemeStyle.ThemeColor }
                          ]}
                        >
                    ...
                    <View style={styles.itemView}>
                      {spec_list_item.value_list.map(
                        (spec_value_list_item, j) => {
                          const selected_index = spec_value_sign.indexOf(
                            spec_value_list_item.id
                          );
                            ...
                            <TouchableOpacity
                              ...
                              onPress={() => {
                                ...
                                if (family_selected_data && !selected) {
                                  const brother_selected_index = spec_value_sign.indexOf(
                                    family_selected_data.id
                                  );
                                  new_spec_value_sign.splice(
                                    brother_selected_index,
                                    1,
                                    spec_value_list_item.id
                                  );
                                } else {
                                  if (selected) {
                                    ...
                                    new_spec_value_sign.push(
                                      spec_value_list_item.id
                                    );
                                  }
                                }
                                const current_skus = skus.find(
                                  (item, index) => {
                                    return (
                                      item.spec_value_sign ===
                                      JSON.stringify(
                                        new_spec_value_sign.sort((a, b) => {
                                          return a - b;
                                        })
                                      )
                                    );
                                ...
                                this.setState({
                                  spec_value_sign: new_spec_value_sign.sort(
                                    (a, b) => {
                                      return a - b;
                                    }
                                  ), // 升序
