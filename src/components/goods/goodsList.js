...
export default class PageGoodsList extends Component {
  render() {
    ...
    return (
      <View style={styles.goodsListWarp}>
        {data.map((item, index) => {
          const params = {
            item,
            index,
    ...
    return (
      ...
      >
        ...
        <View style={styles.bigBot}>
          ...
          <View>
            <Text style={styles.bigMarketPriceText}>
              {showMarketPrice ? `￥${item.market_price}` : ""}
            </Text>
            <Text style={styles.bigPriceText}>
    ...
    return (
      ...
      >
        ...
        <View style={styles.smallBot}>
          ...
          <View>
            <Text style={styles.smallMarketPriceText}>
              {showMarketPrice ? `￥${item.market_price}` : ""}
            </Text>
  ...
  oneBigTwoSmall({ item, index, showTitle, showPrice, showMarketPrice }) {
    const width =
      (index + 1) % 3 === 0 || (index + 1) % 3 === 2
        ? (windowWidth - 10 - 10 * 2) / 2
        : windowWidth - 10 * 2;
    const height =
      (index + 1) % 3 === 0 || (index + 1) % 3 === 2
        ? (windowWidth - 10 - 10 * 2) / 2
        : (windowWidth - 10 * 2) * 0.88;
    return (
      <TouchableOpacity
        key={index}
        onPress={() => this.goDetail(item.id)}
    ...
    return (
      ...
      >
        ...
        <View style={styles.listRight}>
          ...
          <View>
            <Text style={styles.listMarketPriceText}>
              {showMarketPrice ? `￥${item.market_price}` : ""}
            </Text>
