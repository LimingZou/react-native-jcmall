...
export default class CountDown extends Component {
  ...
  render() {
    const { numStyle, symbolStyle } = this.props;
    const { timeItems } = this.state;
    return (
      <View style={styles.countdown}>
        {timeItems && timeItems.length > 0
          ? timeItems.map((item, index) => (
              <View style={styles.item} key={index}>
                <Text style={[styles.itemNum, numStyle]}>{item.num}</Text>
                <Text style={[styles.itemSymbol, symbolStyle]}>{item.symbol}</Text>
              </View>
            ))
          : null}
      </View>
