...
export default class Stepper extends Component {
  ...
  get value() {
    return this.props.value === undefined || this.state.useInput ? this.state.value : this.props.value;
  }
  ...
  onGeneralButtonPress(text) {
    let value = +text;
    let { max, onChange, min } = this.props;
    if (value > max) {
      value = max;
    }
    if (value < min) {
  ...
  renderValue() {
    ...
    return (
      <TextInput
        ...
        onEndEditing={() => {
          this.onGeneralButtonPress(this.state.value);
