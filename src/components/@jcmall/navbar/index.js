...
import NavigationTitle from "./navTitle";
import NavigationButton from "./navButton";
import NavigationLinkButton from "./navLinkButton";
import NavigationIconButton from "./navIconButton";
import NavigationBackButton from "./navBackButton";
import SegmentedItem from "../segmentedBar/item";
...
export default class NavigationBar extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      leftViewWidth: 0,
      rightViewWidth: 0,
      barTop: new Animated.Value(
        props.hidden ? -(Theme.navBarContentHeight + Theme.statusBarHeight) : 0
      ),
    ...
    style = [
      {
        ...
        height:
          ...
          (statusBarInsets ? Theme.statusBarHeight : 0),
  ...
  renderStatusBar(fs) {
    ...
    if (!statusBarColor) {
      statusBarColor =
        statusBarInsets && (Platform.OS === "ios" || Platform.Version > 20)
          ? "rgba(0,0,0,0)"
          : fs.backgroundColor;
    }
    if (!statusBarStyle) {
  ...
  renderBackground() {
    ...
    return (
      <Animated.View style={backgroundViewStyle}>
    ...
    if (typeof title === "string") {
      ...
      title = (
        <this.constructor.Title
          style={[{ textAlign, color: Theme.navTitleColor }].concat(titleStyle)}
          text={title}
        />
      );
    }
  ...
  renderLeftView() {
    ...
    return (
      <Animated.View
        style={{ opacity, paddingLeft: 10 }}
        onLayout={e => this.onLeftViewLayout(e)}
      >
        {leftView}
      </Animated.View>
    );
  ...
  renderRightView() {
    ...
    return (
      <Animated.View
        style={{ opacity, paddingRight: 10 }}
        onLayout={e => this.onRightViewLayout(e)}
      >
        {rightView}
      </Animated.View>
    );
