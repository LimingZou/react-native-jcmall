...
export class SwiperTab extends Component {
  static propTypes = {
    ...
    paginationStyle: PropTypes.oneOfType([PropTypes.number, PropTypes.object]),
        ...
        >
          {(() => {
            ...
            let MaxPage = Math.ceil(
              this.props.dataSource.length / this.state.page
            );
            for (
              var i = 0;
              i < Math.ceil(this.props.dataSource.length / this.state.page);
              i++
            ) {
              buttonView.push(
                <View style={this.state.NavButtonViewStyle} key={i}>
                  {(() => {
                    let button = [];
                    let OnePageNum = 0;
                    ...
                    for (let j = i * this.state.page; j < OnePageNum; j++) {
                      let NewJ = j;

                      button.push(
                        this.state.ButtonFun(this.props.dataSource[NewJ], NewJ)
                      );
                    }
                    return button;
                  })()}
    ...
    return (
      ...
      >
        ...
        >
          ...
          />
          <View style={[styles.ModalMainView, { width: this.state.width }]}>
            {this.props.children}
          </View>
        </View>
      </Modal>
...
export class CountdownButton extends Component {
  ...
  render() {
    const { api, getParams, style, textStyle, verify } = this.props;
    return (
      <Button
...
export class StatusBarComponent extends Component {
  ...
  render() {
    return <StatusBar barStyle={this.state.barStyle} animated={false} />;
  }
}
  ...
  render() {
    ...
    return (
      <View style={[styles.emptyView]}>
        <Image source={uri} style={styles.emptyImg} />
