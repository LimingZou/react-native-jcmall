...
export default class MyWithdraw extends Component {
  ...
  _cashUserAccount = async () => {
    ...
    const params = {
      ...
      bankId:this.state.defaultBankData.id,
    };
    ...
    const e = await Fetch.fetch({
      api: LocalLifeApi.cashUserAccount,
      params
    });
    console.log('提现结果',e);
    if (fa.code.isSuccess(e.code)) {
          ...
          <View style={styles.content}>
            {this.state.isEmptyBankCard===true?
              <TouchableOpacity style={{width:windowWidth-30,marginTop:15,marginLeft:15,height:100,backgroundColor:'white',borderRadius:5,borderColor:'#D9D9D9',borderWidth:1}}
                                onPress={()=>{this.props.navigation.navigate("LocalAddBankCard",{
                                  callback:(()=>{
                                    this.personGetDefaultCard()
                                  })
                                })}}
              ...
              <WithDrawItem
                ...
                backColor="white"
                onPress={() => {
                  this.props.navigation.navigate("LBankcardManage", {
                    callback:((item)=>{
