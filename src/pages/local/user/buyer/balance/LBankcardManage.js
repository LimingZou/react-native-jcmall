...
export default class LBankCardManage extends Component {
  ...
  //     id:item.id,
  //   };
  //   console.log('设置默认银行卡参数',params);
  //   const e = await Fetch.fetch({
  //     api: LocalLifeApi.personSetDefaultCard,
  //     params
  //   });
  //   console.log('设置默认银行卡结果',e);
  //   if (fa.code.isSuccess(e.code)) {
  ...
  manageBankCard(buttonText) {
    if (buttonText == "添加银行卡") {
      if(this.state.isCanClick===true){
        this.props.navigation.navigate("LocalAddBankCard", {
          callback:(()=>{
            this.personQueryMyBankList()
          })
        });
